    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
                        ...
                        if(achou_grupo) {
                            ...
                            if(dias>0) {
                                if(plugin.flatfile) {
                                    ...
                                    plugin.saveConfig();
                                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("addvip").trim().replaceAll("%days%", Integer.toString(dias).replaceAll("%group%", grupo))+".");
                                    plugin.reloadConfig();
                                }
                                else {
                                ...
                                sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error1")+".");
                        }
                        else
                            sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error8")+".");
                    }
                    catch(NumberFormatException e) {
                        sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error2")+".");
                    }
                }
                else
                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+"/addvip <"+plugin.getMessage("group")+"> <"+plugin.getMessage("days")+">");
            }
            else
                sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error11")+"!");
            return true;
        }
            ...
            if(sender.hasPermission("vipzero.darvip")||sender.hasPermission("vipzero.givevip")||sender.hasPermission("vipzero.admin")||sender.isOp()||sender==plugin.getServer().getConsoleSender()) {
                if(args.length==3) {
                    ...
                    if(p!=null) {
                                ...
                                if(dias>0&&dias<100000) {
                                    if(plugin.flatfile) {
                                        if(plugin.usekey_global)
                                            plugin.getServer().broadcastMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("success3").trim().replaceAll("%name%", p.getName()).replaceAll("%group%", grupo).replaceAll("%days%", Integer.toString(dias))+"!");
                                        else
                                            p.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("success2").replaceAll("%group%", grupo.toUpperCase()).replaceAll("%days%", Integer.toString(dias))+"!");
                        ...
                        if(sender!=plugin.getServer().getConsoleSender())
                            sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error7")+"!");
                        else
                    ...
                    if(sender!=plugin.getServer().getConsoleSender())
                        sender.sendMessage(plugin.server_name+ChatColor.RED+"/"+(plugin.getLanguage().trim().equalsIgnoreCase("br") ? "darvip" : "givevip")+" <"+plugin.getMessage("name")+"> <"+plugin.getMessage("group")+"> <"+plugin.getMessage("days")+">");
                    else
                ...
                if(args.length>0&&args.length<4) {
                    ...
                    if(p!=null) {
                        if(args.length<3) {
                            if(plugin.flatfile) {
                                if(plugin.getConfig().contains("vips."+plugin.getRealName(p.getName()))) {
                                    sender.sendMessage(ChatColor.DARK_AQUA+"["+plugin.server_name+"] "+p.getName()+" - "+plugin.getMessage("message2")+":");
                                    ...
                                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+p.getName()+" "+plugin.getMessage("error9")+"!");
                            }
                            else {
                                    ...
                                    if(achou) {
                                        try {
                                            ...
                                            if(dias>1&&dias<100000) {
                                                plugin.getConfig().set("vips."+plugin.getRealName(p.getName())+"."+grupo,dias);
                                                plugin.saveConfig();
                                                plugin.getServer().broadcastMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("cdays").trim().replaceAll("%admin%", sender.getName()).replaceAll("%group%", grupo).replaceAll("%name%", p.getName()).replaceAll("%days%", Integer.toString(dias))+"!");
                                            }
                                            else
                                                sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error1")+"!");
                                        }
                                        catch(Exception e) {sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error8")+"!");}
                                    }
                                    else
                                        sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error2")+"!");
                                }
                    ...
                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+"/"+(plugin.getLanguage().trim().equalsIgnoreCase("br") ? "mudardias" : "changedays")+" <"+plugin.getMessage("name")+"> <"+plugin.getMessage("group")+"> <"+plugin.getMessage("days")+">");
            }
            else
                sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error11")+"!");
            return true;
        ...
        if(cmd.getName().equalsIgnoreCase("trocarvip")||cmd.getName().equalsIgnoreCase("changevip")) {
            if(sender.hasPermission("vipzero.trocarvip")||sender.hasPermission("vipzero.changevip")||sender.isOp()||sender.hasPermission("vipzero.user")||sender.isOp()||sender.hasPermission("vipzero.admin")) {
                if(args.length==1) {
                                ...
                                if(!blockuso) {
                                    if(plugin.getConfig().contains("vips."+sender.getName()+"."+grupo)) {
                                        if(plugin.getConfig().getInt("vips."+sender.getName()+"."+grupo)>0) {
                                            ...
                                            plugin.saveConfig();
                                            sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("success4")+"!");
                                        }
                                        else
                                            sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error12")+"!");
                                    }
                                    else
                                        sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error12")+"!");
                                }
                                else
                                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error10")+"!");
                            }
                            else
                                sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error8")+"!");
                        }
                        else
                            sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error6")+"!");
                    }
                    else {
                        boolean achou = false;
                        String grupo = "";
                    ...
                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+"/"+(plugin.getLanguage().trim().equalsIgnoreCase("br") ? "trocarvip" : "changevip")+" <"+plugin.getMessage("group")+">");
            }
            else
                sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error11")+"!");
            return true;
        ...
        if(cmd.getName().equalsIgnoreCase("gerarkey")||cmd.getName().equalsIgnoreCase("newkey")) {
            if(sender.hasPermission("vipzero.gerarkey")||sender.hasPermission("vipzero.newkey")||sender.hasPermission("vipzero.admin")||sender.isOp()) {
                if(args.length==2) {
                        ...
                        if(achou_grupo) {
                            ...
                            if(dias>0) {
                                ...
                                if(plugin.flatfile) {
                                    ...
                                    plugin.saveConfig();
                                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+"Key: "+ChatColor.GREEN+key+ChatColor.WHITE+" ("+grupo.toUpperCase()+") - "+ChatColor.GREEN+dias+ChatColor.WHITE+" "+plugin.getMessage("message1")+".");
                                    plugin.reloadConfig();
                                }
                                else {
                    ...
                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+"/"+(plugin.getLanguage().trim().equalsIgnoreCase("br") ? "gerarkey" : "newkey")+" <"+plugin.getMessage("group")+"> <"+plugin.getMessage("days")+">");
            }
            else
                sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error11")+"!");
            return true;
            ...
            if(sender.hasPermission("vipzero.keys")||sender.isOp()||sender.hasPermission("vipzero.admin")) {
                if(plugin.flatfile) {
                    if(plugin.getConfig().contains("keys")) {
                        ...
                        if(keys.size()==0)
                            sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error3")+".");
                        else {
                            sender.sendMessage(plugin.server_name+plugin.getMessage("list")+":");
                            for(String n : keys) {
                        ...
                        sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error4")+".");
                }
                else {
        ...
        if(cmd.getName().equalsIgnoreCase("apagarkeys")||cmd.getName().equalsIgnoreCase("delkeys")) {
            if(sender.hasPermission("vipzero.apagarkeys")||sender.hasPermission("vipzero.delkeys")||sender.isOp()||sender.hasPermission("vipzero.admin")) {
                if(plugin.flatfile) {
                    ...
                    plugin.saveConfig();
                    plugin.reloadConfig();
                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("success1")+"!");
                }
                else {
        ...
        if(cmd.getName().equalsIgnoreCase("apagarkey")||cmd.getName().equalsIgnoreCase("delkey")) {
            if(sender.hasPermission("vipzero.apagarkey")||sender.hasPermission("vipzero.delkey")||sender.isOp()||sender.hasPermission("vipzero.admin")) {
                if(args.length==0) {
                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+"/"+(plugin.getLanguage().trim().equalsIgnoreCase("br") ? "apagarkey" : "delkey")+" <key>");
                    return true;
                }
                String key = args[0].toUpperCase();
                if(plugin.flatfile) {
                    if(!plugin.getConfig().contains("keys."+key)) {
                        sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error5")+"!");
                        return true;
                    }
                    plugin.getConfig().set("keys."+key, null);
                    plugin.saveConfig();
                    plugin.reloadConfig();
                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("success5").replaceAll("%key%", key)+"!");
                }
                else {
                    ...
                    if(!plugin.using_codes.containsKey(key)) {
                        ...
                        if(plugin.flatfile) {
                            if(plugin.getConfig().contains("keys."+key)) {
                                ...
                                if(plugin.usekey_global)
                                    plugin.getServer().broadcastMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("success3").trim().replaceAll("%name%", sender.getName()).replaceAll("%group%", grupo).replaceAll("%days%", Integer.toString(dias))+"!");
                                else
                                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("success2").replaceAll("%group%", grupo).replaceAll("%days%", Integer.toString(dias))+"!");
                                if(!plugin.getConfig().contains("vips."+sender.getName())) {
                                    Calendar now = Calendar.getInstance();
                                    SimpleDateFormat fmt = new SimpleDateFormat("dd/MM/yyyy");
                        ...
                        sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error13")+"!");
                }
                else
                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+"/"+(plugin.getLanguage().trim().equalsIgnoreCase("br") ? "usarkey" : "usekey")+" <key>");
            }
            else
                sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error11")+"!");
            return true;
        ...
        if(cmd.getName().equalsIgnoreCase("tempovip")||cmd.getName().equalsIgnoreCase("viptime")) {
            if(sender.hasPermission("vipzero.tempovip")||sender.hasPermission("vipzero.viptime")||sender.hasPermission("vipzero.user")||sender.isOp()||sender.hasPermission("vipzero.admin")) {
                if(plugin.flatfile) {
                    if(plugin.getConfig().contains("vips."+sender.getName())) {
                        sender.sendMessage(ChatColor.DARK_AQUA+"["+plugin.server_name+"] "+plugin.getMessage("message2")+":");
            ...
            if(sender.hasPermission("vipzero.reload")||sender.isOp()||sender.hasPermission("vipzero.admin")) {
                if(args.length==1)
                    if(args[0].equalsIgnoreCase("reload")) {
                        ...
                        sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("reload")+"!");
                        return true;
                    }
            }
            if(plugin.getLanguage().trim().equalsIgnoreCase("br")) {
                sender.sendMessage(plugin.server_name+ChatColor.DARK_AQUA+"Comandos do VipZero:");
                if(sender.hasPermission("vipzero.usarkey")||sender.hasPermission("vipzero.usekey")||sender.hasPermission("vipzero.user")||sender.isOp()||sender.hasPermission("vipzero.admin"))
                ...
                sender.sendMessage(plugin.server_name+ChatColor.DARK_AQUA+"Commands of VipZero:");
                if(sender.hasPermission("vipzero.usarkey")||sender.hasPermission("vipzero.usekey")||sender.hasPermission("vipzero.user")||sender.isOp()||sender.hasPermission("vipzero.admin"))
        ...
        if(cmd.getName().equalsIgnoreCase("tirarvip")||cmd.getName().equalsIgnoreCase("rvip")) {
            if(sender.hasPermission("vipzero.tirarvip")||sender.hasPermission("vipzero.rvip")||sender.isOp()||sender.hasPermission("vipzero.admin")) {
                if(args.length==1) {
                    ...
                    if(p!=null) {
                        if(plugin.flatfile) {
                            if(plugin.getConfig().contains("vips."+plugin.getRealName(p.getName()))) {
                                ...
                                plugin.getServer().broadcastMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("rvip").trim().replaceAll("%admin%", sender.getName()).replaceAll("%name%", p.getName())+"!");
                            }
                            else
                                sender.sendMessage(plugin.server_name+ChatColor.WHITE+p.getName()+" "+plugin.getMessage("error9")+"!");
                        }
                    ...
                    sender.sendMessage(plugin.server_name+ChatColor.WHITE+"/"+(plugin.getLanguage().trim().equalsIgnoreCase("br") ? "tirarvip" : "rvip")+" <"+plugin.getMessage("name")+">");
            }
            else
                sender.sendMessage(plugin.server_name+ChatColor.WHITE+plugin.getMessage("error11")+"!");
            return true;
